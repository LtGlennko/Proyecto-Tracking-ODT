...
-- sla_config: al menos 1 dimensión debe ser NOT NULL
ALTER TABLE sla_config
  ADD CONSTRAINT chk_sla_min_dimension
  CHECK (
    empresa_id IS NOT NULL OR
    subetapa_id IS NOT NULL OR
    linea_negocio IS NOT NULL OR
    tipo_vehiculo IS NOT NULL
  );

-- chat: XOR — solo ficha_id O vin_id, nunca ambos ni ninguno
ALTER TABLE chat
...
ALTER TABLE vin_hito_tracking
  ADD CONSTRAINT uq_vin_hito UNIQUE (vin_id, hito_id);

ALTER TABLE vin_subetapa_tracking
  ADD CONSTRAINT uq_vin_subetapa UNIQUE (vin_id, subetapa_id);

ALTER TABLE usuario_empresa
  ADD CONSTRAINT uq_usuario_empresa UNIQUE (usuario_id, empresa_id);

-- ── ÍNDICES DE PERFORMANCE ──────────────────────────────────
CREATE INDEX IF NOT EXISTS idx_vin_ficha_id           ON vin(ficha_id);
CREATE INDEX IF NOT EXISTS idx_vin_tipo_vehiculo      ON vin(tipo_vehiculo);
CREATE INDEX IF NOT EXISTS idx_vin_linea_negocio      ON vin(linea_negocio);
CREATE INDEX IF NOT EXISTS idx_cliente_empresa_id     ON cliente(empresa_id);
CREATE INDEX IF NOT EXISTS idx_ficha_cliente_id       ON ficha(cliente_id);
CREATE INDEX IF NOT EXISTS idx_vht_vin_id             ON vin_hito_tracking(vin_id);
CREATE INDEX IF NOT EXISTS idx_vht_hito_id            ON vin_hito_tracking(hito_id);
CREATE INDEX IF NOT EXISTS idx_vht_estado             ON vin_hito_tracking(estado);
CREATE INDEX IF NOT EXISTS idx_vst_vin_id             ON vin_subetapa_tracking(vin_id);
CREATE INDEX IF NOT EXISTS idx_vst_subetapa_id        ON vin_subetapa_tracking(subetapa_id);
CREATE INDEX IF NOT EXISTS idx_vst_estado             ON vin_subetapa_tracking(estado);
CREATE INDEX IF NOT EXISTS idx_alerta_vin_id          ON alerta(vin_id);
CREATE INDEX IF NOT EXISTS idx_alerta_nivel_estado    ON alerta(nivel, estado_alerta);
CREATE INDEX IF NOT EXISTS idx_sla_empresa_subetapa   ON sla_config(empresa_id, subetapa_id);
CREATE INDEX IF NOT EXISTS idx_mensaje_chat_id        ON mensaje(chat_id);
CREATE INDEX IF NOT EXISTS idx_staging_pedido_externo ON staging_vin(pedido_externo);
CREATE INDEX IF NOT EXISTS idx_staging_fuente_sync    ON staging_vin(fuente_ultima_sync);
...
FROM (VALUES
  ('Importación',     'financiero', 1, 'Bloque Principal Financiero + PDI', NULL),
...
FROM (VALUES
  ...
  -- PDI (solo buses — activo_default = FALSE)
