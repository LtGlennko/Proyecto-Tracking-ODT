  ...
  template: `
    <div class="bg-white rounded-lg border border-slate-200 p-6 overflow-x-auto">
      ...
      <div class="flex items-center gap-2 flex-nowrap min-w-max mb-8">
        <ng-container *ngFor="let hito of mainFlow(); let last = last">
          <kf-stage-node [hito]="hito" (nodeClick)="nodeClick.emit($event)" />
          <div *ngIf="!last" class="flex items-center flex-shrink-0">
      ...
      <div *ngIf="parallelFlow().length > 0" class="ml-16 border-l-2 border-dashed border-slate-200 pl-6">
        <p class="text-xs text-slate-400 mb-3">Paralelo</p>
        <div class="flex items-center gap-3 flex-wrap">
          <ng-container *ngFor="let hito of parallelFlow(); let last = last">
            <kf-stage-node [hito]="hito" (nodeClick)="nodeClick.emit($event)" />
            <div *ngIf="!last" class="flex items-center">
